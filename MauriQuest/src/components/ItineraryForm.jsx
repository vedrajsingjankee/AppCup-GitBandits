...
const ItineraryForm = ({ onItineraryGenerated, isModal = false, onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...formData,
        location: formData.region ? formData.region.charAt(0).toUpperCase() + formData.region.slice(1) : undefined,
        activityTypes: formData.interests,
        accessible: Boolean(formData.disabilityAccess)
      };
      ...
      onItineraryGenerated({ ...data, preferences: formData });
  ...
  const handleGetTopRecommendations = async () => {
    ...
    try {
      ...
      onItineraryGenerated({ summary: 'Top Recommendations', recommendations: recs, preferences: formData });
      if (isModal && onClose) {
        onClose();
      }
    } catch (err) {
    ...
    <div className={`${isModal ? 'fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4' : 'w-full max-w-2xl mx-auto'}`}>
      <div className={`${isModal 
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <button
            type="button"
            onClick={handleGetTopRecommendations}
            disabled={loading}
            className="w-full bg-gradient-to-r from-blue-500 to-cyan-500 text-white py-2.5 px-4 rounded-lg font-semibold hover:from-blue-600 hover:to-cyan-600 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed text-sm"
          >
            {loading ? 'Loading...' : 'Get Top Recommendations'}
          </button>
        </form>
      </div>
    </div>
