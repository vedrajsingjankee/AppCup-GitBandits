...
const ASLDemo = () => {
  ...
  // Same-origin by default; set VITE_API_BASE_URL if needed
  ...
  // Cache phrases only; reveal after video ends
  ...
  const fetchDemoPhrases = async () => {
    const url = joinUrl(API_BASE_URL, "/api/demo-asl");
    console.log("📡 Fetching demo phrases from:", url);
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const url = joinUrl(API_BASE_URL, "/api/upload-asl");
      ...
      if (data.file?.url) {
        ...
        if (previewRef.current) {
          const videoSrc = joinUrl(API_BASE_URL, data.file.url);
          previewRef.current.src = videoSrc;
  ...
  // Reveal results AFTER video ends
  const handleEnded = async () => {
    try {
      await ensurePhrasesReady(); // ensure phrases exist even if user skipped
      setShowResult(true);
      setStatus("", "muted"); // clear status line
    } catch {
      setStatus("Could not load subtitles.", "muted err");
    }
  };

  // Health check on mount — logs only (no UI text)
  useEffect(() => {
    const url = joinUrl(API_BASE_URL, "/health");
    console.log("🩺 Health ping to:", url);
    (async () => {
      try {
        ...
        // (Intentionally not calling setStatus here to avoid showing a green line)
      } catch (e) {
        console.warn("⚠️ Backend health check failed:", e);
        // Also not setting UI status here, per your request
      }
    })();
  }, [API_BASE_URL]);
